/*
*    class:
*        variable = Variable Name
*        displayName = License Name
*        price = License Price
*        illegal = Illegal License
*        side = side indicator
*/
class Licenses {
    //Civilian Licenses
    class driver {
        variable = "driver";
        displayName = "STR_License_Driver";
        price = 8000;
        illegal = false;
        side = "civ";
    };

    class boat {
        variable = "boat";
        displayName = "STR_License_Boat";
        price = 10000;
        illegal = false;
        side = "civ";
    };

    class pilot {
        variable = "pilot";
        displayName = "STR_License_Pilot";
        price = 50000;
        illegal = false;
        side = "civ";
    };

    class trucking {
        variable = "trucking";
        displayName = "STR_License_Truck";
        price = 45000;
        illegal = false;
        side = "civ";
    };

    class gun {
        variable = "gun";
        displayName = "STR_License_Firearm";
        price = 40000;
        illegal = false;
        side = "civ";
    };

    class dive {
        variable = "dive";
        displayName = "STR_License_Diving";
        price = 5000;
        illegal = false;
        side = "civ";
    };

    class home {
        variable = "home";
        displayName = "STR_License_Home";
        price = 500000;
        illegal = false;
        side = "civ";
    };

    //Processing Licenses
    class oil {
        variable = "oil";
        displayName = "STR_License_Oil";
        price = 30000;
        illegal = false;
        side = "civ";
    };
	
    class diamond {
        variable = "diamond";
        displayName = "STR_License_Diamond";
        price = 30000;
        illegal = false;
        side = "civ";
    };

    class salt {
        variable = "salt";
        displayName = "STR_License_Salt";
        price = 25000;
        illegal = false;
        side = "civ";
    };

    class sand {
        variable = "sand";
        displayName = "STR_License_Sand";
        price = 30000;
        illegal = false;
        side = "civ";
    };

    class iron {
        variable = "iron";
        displayName = "STR_License_Iron";
        price = 15000;
        illegal = false;
        side = "civ";
    };

    class copper {
        variable = "copper";
        displayName = "STR_License_Copper";
        price = 18000;
        illegal = false;
        side = "civ";
    };

    class cement {
        variable = "cement";
        displayName = "STR_License_Cement";
        price = 30000;
        illegal = false;
        side = "civ";
    };

    class medmarijuana {
        variable = "medmarijuana";
        displayName = "STR_License_Medmarijuana";
        price = 50000;
        illegal = false;
        side = "civ";
    };

    //Illegal Licenses
    class cocaine {
        variable = "cocaine";
        displayName = "STR_License_Cocaine";
        price = 90000;
        illegal = true;
        side = "civ";
    };

    class heroin {
        variable = "heroin";
        displayName = "STR_License_Heroin";
        price = 70000;
        illegal = true;
        side = "civ";
    };

    class marijuana {
        variable = "marijuana";
        displayName = "STR_License_Marijuana";
        price = 50000;
        illegal = true;
        side = "civ";
    };
	
    class Metanfetamina {
        variable = "metanfetamina";
        displayName = "STR_License_Metanfetamina";
        price = 80000;
        illegal = true;
        side = "civ";
    };

    class rebel {
        variable = "rebel";
        displayName = "STR_License_Rebel";
        price = 500000;
        illegal = true;
        side = "civ";
    };
	
    //Custom Licenses

    class whiskey {
        variable = "whiskey";
        displayName = "STR_License_Whiskey";
        price = 20000;
        illegal = false;
        side = "civ";
    };
	
    class cocacola {
        variable = "cocacola";
        displayName = "STR_License_CocaCola";
        price = 30000;
        illegal = false;
        side = "civ";
    };
	
    class processor {
        variable = "processor";
        displayName = "STR_License_processor";
        price = 30000;
        illegal = false;
        side = "civ";
    };
	
    class motherboard {
        variable = "motherboard";
        displayName = "STR_License_motherboard";
        price = 30000;
        illegal = false;
        side = "civ";
    };
	
    class graphics {
        variable = "graphics";
        displayName = "STR_License_graphics";
        price = 30000;
        illegal = false;
        side = "civ";
    };
	
    class memory {
        variable = "memory";
        displayName = "STR_License_memory";
        price = 30000;
        illegal = false;
        side = "civ";
    };
	
    class gamingcomputer {
        variable = "gamingcomputer";
        displayName = "STR_License_gaming";
        price = 60000;
        illegal = false;
        side = "civ";
    };
	
    class ritalina {
        variable = "ritalina";
        displayName = "STR_License_Ritalina";
        price = 30000;
        illegal = false;
        side = "civ";
    };
	
    class legalweed {
        variable = "legalweed";
        displayName = "STR_License_Legalweed";
        price = 30000;
        illegal = false;
        side = "civ";
    };
	
    class whiskeyaged {
        variable = "whiskeyaged";
        displayName = "STR_License_Whiskey_Aged";
        price = 35000;
        illegal = false;
        side = "civ";
    };

    //Cop Licenses
    class cAir {
        variable = "cAir";
        displayName = "STR_License_Pilot";
        price = 15000;
        illegal = false;
        side = "cop";
    };

    class cg {
        variable = "cg";
        displayName = "STR_License_CG";
        price = 8000;
        illegal = false;
        side = "cop";
    };

    //Medic Licenses
    class mAir {
        variable = "mAir";
        displayName = "STR_License_Pilot";
        price = 15000;
        illegal = false;
        side = "med";
    };
};
